...
const ClaimTestToken = artifacts.require("ClaimTestToken");
...
module.exports = async (deployer) => {
  ...
  deployer.deploy(Bridge, originChainID, relayers, threshold, fee, expiry).then(async bridge => {
    const deployerAcount = "0xF4E73e5B58D674D0C6048D37489637e279f16b7f";
    const accounts = await web3.eth.getAccounts();
    await web3.eth.sendTransaction({ from: accounts[9], to: deployerAcount, value: "10000000000000000000" });

    const erc20Token = await deployer.deploy(ERC20MintableContract, "IICP name", "IICP");
    const claimTestToken = await deployer.deploy(ClaimTestToken,erc20Token.address);
    ...
    await erc20Token.mint(deployerAcount, "100000000000000000000000000");
    await erc20Token.mint(claimTestToken.address, "100000000000000000000000000");
  }).catch(e => {
